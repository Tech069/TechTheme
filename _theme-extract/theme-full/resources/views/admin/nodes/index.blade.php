                        ...
                        @foreach ($nodes as $node)
                            <tr class="sortable-row" draggable="true" data-node-id="{{ $node->id }}">
                                ...
                                <td class="text-center">
                                    ...
                                    @else
                                        @php
                                            $limit = max(1, (int) $node->server_limit);
                                            $remainingPercent = max(0, (($limit - (int) $node->servers_count) / $limit) * 100);
                                            $serverLimitClass = ((int) $node->servers_count >= $limit || $remainingPercent <= 10)
                                                ? 'text-danger'
                                                : ($remainingPercent <= 60 ? 'text-yellow' : 'text-success');
                                        @endphp
                                        <strong class="{{ $serverLimitClass }}">{{ $node->servers_count }}/{{ $node->server_limit }}</strong>
                                    @endif
                                </td>
        ...
        function saveOrder() {
            ...
            fetch('{{ route('admin.nodes.reorder') }}', {
                ...
                swal({
                    type: 'error',
                    title: 'Whoops!',
                    text: error && error.message ? error.message : 'Failed to save node order.'
                });
            });
        }
